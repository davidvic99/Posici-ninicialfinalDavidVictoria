...
function casillaAleatoria(posInicial, posFinal) {
    ...
    for (let i = 0; i < 2; i++) {
        ...
        posicioninicial = Math.floor(Math.random() * (filalength));
        posInicial.push(posicioninicial);
        
    }

    for (let i = 0; i < 2; i++) {

        posicionfinal = Math.floor(Math.random() * (filalength));
        posFinal.push(posicioninicial);
